...
class Package:
    def __init__(self,
                 ...
                 depends: typing.Set["natup_pkg.Package"] = None,
                 install_func: typing.Callable[["Package", "natup_pkg.Environment", str], None] = None,
                 build_func: typing.Callable[["natup_pkg.Package", "natup_pkg.Environment"], None] = None):
        self.name = name
        ...
        if not os.path.exists(archive_path):
            with env.tmp_swap_file(archive_path) as tmp_path:
                ...
                if self.archive_hash == 'none':
                    logging.warning("skipping hash check for package: %s, version: %s", self.name, self.version_str)
    ...
    def get_archive_path(self, env: "natup_pkg.Environment") -> str:
        return env.get_archive_dir() + "/" + self.name + "_" + self.version_str + ".tar.gz"

    def get_src_dir(self, env: "natup_pkg.Environment") -> str:
        return env.get_src_dir() + "/" + self.name + "_" + self.version_str

    def get_build_dir(self, env: "natup_pkg.Environment") -> str:
        return env.get_build_dir() + "/" + self.name + "_" + self.version_str

    def get_install_dir(self, env: "natup_pkg.Environment"):
        return env.get_install_dir() + "/" + self.name + "_" + self.version_str

    def install(self, env: "natup_pkg.Environment"):
        if not os.path.exists(self.get_install_dir(env)):
            logging.info("Downloading package: %s, version: %s, url: %s", self.name, self.version_str, self.archive_url)
            self.download_archive(env)
            logging.info("Extracting package: %s, version: %s", self.name, self.version_str)
            ...
            if self.build_func is not None:
                logging.info("Building package: %s, version: %s", self.name, self.version_str)
                os.makedirs(self.get_build_dir(env))
                self.build_func(self, env)

            logging.info("Installing package: %s, version: %s", self.name, self.version_str)
            with env.tmp_swap_file(self.get_install_dir(env)) as tmp_dir:
                self.install_func(self, env, tmp_dir)
        else:
            logging.info("Skipping package: %s, version: %s, already installed", self.name, self.version_str)
